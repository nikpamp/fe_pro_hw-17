...
function unique(array) {
    let uniqueArray = array.filter((num, index, array) => array.indexOf(num) === index);
    return uniqueArray;
}

console.log(unique([8, 5, 6, 12, 21, 5, 4, 12]));
...
function anagram(firstWord, secondWord) {
    firstWord = firstWord.toLowerCase().replace(/[.,\/#!$%\^&\*;:{}=\-_`~()]/g,'').replace(/\s+/g,'');
    secondWord = secondWord.toLowerCase().replace(/[.,\/#!$%\^&\*;:{}=\-_`~()]/g,'').replace(/\s+/g,'');
    ...
    if (firstWordArray === secondWordArray) {
        return `These strings are anagrams!`;
    } else return `These strings are not anagrams!`;
}

console.log(anagram('slovo - ne vorobey', 'enl(o)svo!  *rb`ooev,y'));
...
function solution(words) {
    let uniqueArray = Array.from(new Set(words));
    let finalArray = [];

    for (let index = 0; index < uniqueArray.length; index++) {
        finalArray.push(words.filter((word) => word === uniqueArray[index]));
    };
    ...
    return Array.from(new Set(finalArray.flat()));
}
